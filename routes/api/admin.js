...
router.post('/sign-up', privateHeader, async (req, res) => {
  ...
  if (!error) {
    try {
      connect.query('SELECT * FROM admin WHERE email=?', req.body.email, async (error, result) => {
...
router.post('/login', privateHeader, async (req, res) => {
  ...
  if (!error) {
    connect.query('SELECT * FROM admin WHERE email=?', email, async (error, result) => {
      if (!error && result.length > 0) {
        ...
        //Return JwtToken
        const payload = { user: { id: user[0].id, password: user[0].password, role: user[0].role } };
